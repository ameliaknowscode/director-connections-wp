...
class DC_List_Table_Movies extends WP_List_Table {
    ...
    protected function column_cb( $item ) {
        return sprintf( '<input type="checkbox" name="movies[]" value="%d" />', $item->id );
    }
    ...
    protected function column_title( $item ) {
        ...
        $delete_url = wp_nonce_url(
            ...
            'dc_delete_movie_' . $item->id
        );

        $actions = array(
            'edit'   => sprintf( '<a href="%s">Edit</a>', esc_url( $edit_url ) ),
            'delete' => sprintf( '<a href="%s" onclick="return confirm(\'Delete %s?\')">Delete</a>', esc_url( $delete_url ), esc_js( $item->title ) ),
        );
    ...
    public function prepare_items() {
        ...
        $this->items = DC_Movies::all( array(
            'orderby' => $orderby,
            'order'   => $order,
            'search'  => $search,
            'limit'   => $per_page,
